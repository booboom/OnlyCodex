...
class ProxyConfig:
    def __init__(
        ...
        *,
        ...
        timeout_sec: float,
        max_body_bytes: int,
        default_protocol: str = "chat_completions",
        ...
        self.default_protocol = normalize_protocol(default_protocol)
    ...
    def resolve_route(self, model: str) -> tuple[str, str, str, str | None]:
        ...
        # 4. Optional default upstream (legacy CLI / tests). Desktop builds leave this empty.
        if self.chat_base_url:
            upstream = MODEL_ALIASES.get(model, DEFAULT_MODEL)
            return "default", upstream, self.chat_base_url, None
        return "unrouted", model, "", None

    def protocol_for_provider(self, provider_name: str) -> str:
        if provider_name == "default":
            return self.default_protocol
        return normalize_protocol(self.providers.get(provider_name, {}).get("protocol"))
...
def load_provider_config(
    path: str | None,
) -> tuple[dict[str, dict], dict[str, str], dict[str, str], set[str]]:
    ...
    routes: dict[str, str] = {}
    # Only expose models from configured providers/mappings. Legacy DEFAULT_MODEL
    # seeds are intentionally omitted so desktop installs are provider-only.
    models: set[str] = set()
    ...
    for name, p in (data.get("providers") or {}).items():
        ...
        providers[name] = {
            ...
            "protocol": normalize_protocol(p.get("protocol")),
        }
...
def normalize_protocol(value: Any) -> str:
    """Normalize provider protocol names while keeping old configs compatible."""
    value = str(value or "responses").strip().lower().replace("-", "_")
    return "chat_completions" if value in {"chat", "chat_completion", "chat_completions"} else "responses"


def visible_model_ids(config: ProxyConfig) -> list[str]:
    """Expose mapped Codex aliases; retain legacy CLI behavior without mappings."""
    if config.mappings:
        return sorted(config.mappings)
...
class ResponsesProxyHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        ...
        if self.path in {"/models", "/v1/models"}:
            ...
            # Advertise mapped aliases as if they were native Codex models.
            self._send_json(
                {
                    "object": "list",
                    "data": [
                        {"id": slug, "object": "model"}
                        for slug in visible_model_ids(config)
                    ],
                }
            )
            return
        self._send_json(
            {"error": {"message": "not found"}}, status=HTTPStatus.NOT_FOUND
        )
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            _pname, upstream_model, base_url, api_key = config.resolve_route(
                original_model
            )
            if not base_url:
                raise ProxyError(
                    HTTPStatus.BAD_REQUEST,
                    f"no route configured for model '{original_model}'; add a provider mapping or enable a provider model",
                )
            upstream = {"base_url": base_url, "api_key": api_key, "provider": _pname}
            upstream["protocol"] = config.protocol_for_provider(_pname)
            ...
            if payload.get("stream"):
                ...
                try:
                    ...
                    with contextlib.suppress(BrokenPipeError):
                        err = json.dumps(
                            {
                                "type": "response.error",
                                "error": {"message": "proxy crashed; see stderr trace"},
                            },
                            separators=(",", ":"),
                        ).encode("utf-8")
                ...
                response = handle_responses_request(
                    payload, config, request_id, upstream, original_model
                )
            ...
            self._send_json(
                {"error": {"message": exc.message, "type": "proxy_error"}},
                status=exc.status,
            )
            ...
            with contextlib.suppress(BrokenPipeError):
                self._send_json(
                    {
                        "error": {
                            "message": "proxy crashed; see stderr trace",
                            "type": "proxy_crash",
                        }
                    },
    ...
    def _read_json(self, config: ProxyConfig) -> Json:
        ...
        if length > config.max_body_bytes:
            raise ProxyError(
                ...
                f"request body exceeds {config.max_body_bytes // (1024 * 1024)}MB cap",
            )
...
def handle_responses_request(
    ...
    if isinstance(upstream, dict) and upstream.get("protocol") == "responses":
        return call_upstream_responses(payload, config, request_id, upstream=upstream)
    ...
    if conversion_stats.get("has_image") and conversion_stats.get("tools_present"):
        chat_payload = caption_images_in_messages(
            chat_payload, request_model, config, request_id, upstream
        )
...
def handle_streaming_request(
    ...
    if isinstance(upstream, dict) and upstream.get("protocol") == "responses":
        stream_upstream_responses(payload, config, request_id, wfile, upstream=upstream)
        return
    chat_payload, request_model, conversion_stats = responses_payload_to_chat_payload(
        payload
    )
    ...
    try:
        base_url, api_key = resolve_upstream_target(config, request_id, upstream)
    except ProxyError as exc:
    ...
    try:
                    ...
                    send_event(
                        {
                            ...
                            "item_id": message_id,
                            "output_index": 1 if reasoning_open else 0,
                            "delta": d,
                        }
                    )
                    ...
                    for tc in tcs:
                        ...
                        if name_delta:
                            ...
                            if (
                                ...
                                call_id = (
                                    tool_calls[idx]["id"] or f"call_{uuid.uuid4().hex}"
                                )
                                tc_item: Json = {
                                    "type": "function_call",
                                    "id": fc_id,
        ...
        trace(
            "upstream.error", request_id=request_id, status=exc.code, body=body[:2000]
        )
        if exc.code == 429:
    ...
    if reasoning_open:
        ...
        send_event(
            {"type": "response.output_item.done", "output_index": 0, "item": rs_done}
        )
...
def caption_images_in_messages(
    chat_payload: Json,
    target_model: str,
    config: ProxyConfig,
    request_id: str,
    ...
    for message in messages:
        ...
        if isinstance(content, list) and all(
            ...
            message["content"] = "\n".join(
                p.get("text", "") for p in content if p.get("text")
            )
...
def caption_image_via_mimo(
    ...
    try:
        chat = call_upstream_chat(
            caption_payload, config, request_id, timeout_sec=15.0, upstream=upstream
        )
...
def resolve_upstream_target(
    ...
    """Resolve (base_url, api_key) for chat completions calls.

    Custom providers always win. Empty api_key falls back to env/keychain only when
    using the optional default upstream.
    """
    if upstream is not None and upstream.get("base_url"):
        base_url = str(upstream["base_url"]).rstrip("/")
        api_key = upstream.get("api_key")
        if api_key:
            return base_url, str(api_key)
        # Local servers often need no key; still send a placeholder bearer token.
        if upstream.get("provider") not in {None, "default", "unrouted"}:
            return base_url, "not-required"
        return base_url, resolve_api_key(config, request_id)
    if not config.chat_base_url:
        raise ProxyError(
            HTTPStatus.BAD_REQUEST,
            "no upstream configured: add a provider mapping or enable a provider model",
        )
    return config.chat_base_url.rstrip("/"), resolve_api_key(config, request_id)


def upstream_endpoint(base_url: str, endpoint: str) -> str:
    base = base_url.rstrip("/")
    return base if base.endswith(f"/{endpoint}") or base.endswith(endpoint) else f"{base}/{endpoint}"


def call_upstream_responses(
    payload: Json,
    config: ProxyConfig,
    request_id: str,
    *,
    ...
    """Forward a Responses request unchanged to a Responses-native provider."""
    base_url, api_key = resolve_upstream_target(config, request_id, upstream)
    url = upstream_endpoint(base_url, "responses")
    ...
    try:
        with urllib.request.urlopen(request, timeout=config.timeout_sec) as response:
            ...
            if value is _INVALID_JSON or not isinstance(value, dict):
                raise ProxyError(HTTPStatus.BAD_GATEWAY, "upstream Responses returned invalid JSON")
        ...
        raise ProxyError(HTTPStatus.BAD_GATEWAY, f"upstream Responses network error: {exc.reason}") from exc


def stream_upstream_responses(
    payload: Json,
    config: ProxyConfig,
    request_id: str,
    wfile: Any,
    *,
    ...
    """Relay native Responses SSE events without converting their event schema."""
    base_url, api_key = resolve_upstream_target(config, request_id, upstream)
    url = upstream_endpoint(base_url, "responses")
    raw_payload = json.dumps(payload, separators=(",", ":")).encode("utf-8")
    request = urllib.request.Request(
    ...
    try:
        with urllib.request.urlopen(request, timeout=config.timeout_sec) as response:
            for raw_line in response:
                ...
                wfile.flush()
        trace("upstream.responses.done", request_id=request_id, status=200, stream=True)
    except urllib.error.HTTPError as exc:
        body = exc.read().decode("utf-8", errors="replace")
        trace("upstream.responses.error", request_id=request_id, status=exc.code, body=body[:2000])
        with contextlib.suppress(BrokenPipeError):
            event = json.dumps({"type": "response.error", "error": {"message": f"upstream Responses HTTP {exc.code}"}}, separators=(",", ":")).encode()
            wfile.write(b"data: " + event + b"\n\ndata: [DONE]\n\n")
            wfile.flush()
    except (urllib.error.URLError, TimeoutError) as exc:
        trace("upstream.responses.network_error", request_id=request_id, reason=str(getattr(exc, "reason", exc)))
        with contextlib.suppress(BrokenPipeError):
            event = json.dumps({"type": "response.error", "error": {"message": str(getattr(exc, "reason", exc))}}, separators=(",", ":")).encode()
            wfile.write(b"data: " + event + b"\n\ndata: [DONE]\n\n")
            wfile.flush()
    ...
    *,
    ...
    try:
        with urllib.request.urlopen(
            request, timeout=timeout_sec or config.timeout_sec
        ) as response:
            body = response.read()
            ...
            if value is _INVALID_JSON:
                raise ProxyError(
                    HTTPStatus.BAD_GATEWAY, "upstream returned invalid JSON"
                )
            if not isinstance(value, dict):
                raise ProxyError(
                    HTTPStatus.BAD_GATEWAY, "upstream returned non-object JSON"
                )
            return value
    except urllib.error.HTTPError as exc:
        body = exc.read().decode("utf-8", errors="replace")
        ...
        raise ProxyError(
            HTTPStatus.BAD_GATEWAY, f"upstream network error: {exc.reason}"
        ) from exc
...
def resolve_api_key(config: ProxyConfig, request_id: str) -> str:
    ...
    with _api_key_lock:
        ...
        raise ProxyError(
            ...
            f"missing API key: set ${config.api_key_env} or keychain:{keychain_service}",
        )


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Codex Responses API shim for OpenAI Chat Completions upstreams"
    )
    parser.add_argument(
        "--version", action="version", version=f"%(prog)s {__version__}"
    )
    parser.add_argument(
        "--bind", default=os.environ.get("OPENCODE_GO_PROXY_BIND", "127.0.0.1")
    )
    parser.add_argument(
        "--port",
        type=int,
        default=_env_int("OPENCODE_GO_PROXY_PORT", 8787),
    )
    parser.add_argument(
        "--chat-base-url",
        dest="chat_base_url",
        default=os.environ.get("CHAT_COMPLETIONS_BASE_URL", ""),
        help="Optional legacy default Chat Completions base URL (empty = providers only)",
    )
    parser.add_argument(
        "--default-protocol",
        dest="default_protocol",
        default=os.environ.get("OPENCODEX_DEFAULT_PROTOCOL", "responses"),
        choices=["responses", "chat_completions"],
        help="Protocol used by the built-in default provider (default: responses)",
    )
    parser.add_argument(
        "--api-key-env",
        default=os.environ.get("OPENCODE_GO_PROXY_API_KEY_ENV", "OPENCODE_GO_API_KEY"),
    )
    parser.add_argument(
        "--timeout-sec",
        type=float,
        default=_env_float("OPENCODE_GO_PROXY_TIMEOUT_SEC", 180.0),
    )
    parser.add_argument(
        "--max-body-mb",
        type=int,
        default=_env_int("OPENCODE_GO_PROXY_MAX_BODY_MB", 20),
    )
    parser.add_argument(
        "--config",
        dest="config",
        default=os.environ.get(
            "OPENCODEX_CONFIG",
            os.path.expanduser("~/.config/opencodex-proxy/config.json"),
        ),
