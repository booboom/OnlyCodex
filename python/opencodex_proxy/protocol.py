...
def _load_catalog_models() -> set[str]:
    ...
    catalog_path = os.environ.get(
        "CODEX_MODEL_CATALOG",
        os.path.expanduser("~/.codex/model-catalogs/opencodex.json"),
    )
    try:
...
def _is_safe_url_string(s: str) -> bool:
    ...
    return isinstance(s, str) and (
        s.startswith("https://") or (s.startswith("data:image/") and "base64," in s)
    )
...
def _content_to_chat_parts(content: Any) -> list[Json] | str:
    ...
    has_image = any(
        ...
        and part.get("type") in {"input_image", "image_url", "image"}
...
def responses_input_to_chat_messages(payload: Json) -> tuple[list[Json], Json]:
    ...
    for item in input_value:
        ...
        message: Json = {
            "role": role,
            "content": _content_to_chat_parts(item.get("content", "")),
        }
...
def responses_payload_to_chat_payload(payload: Json) -> tuple[Json, str, Json]:
    ...
    incoming_model = payload.get("model") or DEFAULT_MODEL
    # Optional legacy aliases (OpenAI slug -> OpenCodeX). Custom provider model IDs
    # must pass through unchanged so routed upstream names are not rewritten.
