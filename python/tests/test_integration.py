...
def mock_chat_response(
    content: str = "hello", model: str = "deepseek-v4-flash"
) -> dict:
    return {
        "id": "chatcmpl-test",
...
class TestResponsesRoundTrip:
    def test_non_streaming_response(self, server):
        ...
        with mock.patch.dict(os.environ, {"OPENCODE_GO_API_KEY": "test-key"}):
            with mock.patch(
                ...
                return_value=MockUpstreamResponse(json.dumps(mock_resp).encode()),
            ):
                conn = HTTPConnection("127.0.0.1", port, timeout=5)
                conn.request(
                    "POST",
    ...
    def test_missing_api_key_returns_401(self, server):
        ...
        with (
            ...
            mock.patch(
                "opencodex_proxy.app.subprocess.run", return_value=failed_completed
            ),
        ):
            conn = HTTPConnection("127.0.0.1", port, timeout=5)
            conn.request(
...
class TestStreamingResponse:
    ...
    def test_streaming_crash_sends_sse_error(self, server):
        ...
        with mock.patch.dict(os.environ, {"OPENCODE_GO_API_KEY": "test-key"}):
            with mock.patch(
                "opencodex_proxy.app.responses_payload_to_chat_payload",
...
class TestEdgeCases:
    ...
    def test_upstream_invalid_json_returns_502(self, server):
        ...
        with mock.patch.dict(os.environ, {"OPENCODE_GO_API_KEY": "test-key"}):
            with mock.patch(
                "urllib.request.urlopen", return_value=MockUpstreamResponse(b"not json")
            ):
                conn = HTTPConnection("127.0.0.1", port, timeout=5)
                conn.request(
                    "POST",
...
class TestVersionFlag:
    def test_version_flag_prints_version(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "opencodex_proxy", "--version"],
            capture_output=True,
            text=True,
...
class TestStreamingToolCalls:
    def test_streaming_tool_call_sse(self, server):
        ...
        added_events = re.findall(
            r"response\.output_item\.added.*?function_call", raw_text
        )
            ...
            "choices": [
                {
                    ...
                    "message": {
                        "role": "assistant",
                        "content": "A screenshot of a code editor",
                    },
                    "finish_reason": "stop",
                }
            ],
